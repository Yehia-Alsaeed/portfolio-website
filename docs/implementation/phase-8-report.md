...
## Addendum: Production URL renamed after Phase 8

The Production alias was renamed from `portfolio-website-azure-pi.vercel.app` to **`https://yehia-alsaeed.vercel.app`** after this phase closed. The original name carried a Vercel-generated suffix because `portfolio-website.vercel.app` is already claimed by another account; Vercel appends a random word pair when the requested name is taken.

**Every URL recorded earlier in this report and in the Phase 1 and Phase 4 reports refers to the old alias.** Those references are left as written — they record what was actually measured at the time, and rewriting them would misrepresent the evidence. The old alias was deleted rather than kept as a redirect, so it now returns 404.

The rename exposed the build-time inlining behaviour a second time, in the same shape as the `NEXT_PUBLIC_SITE_URL` gap documented above. The first redeploy after adding the new domain reused the existing build cache, so the compiled output still carried the old origin: the site answered on the new URL while advertising `rel="canonical"`, `og:image`, `robots.txt` `Host`/`Sitemap`, and every `sitemap.xml` entry as the old — now 404 — address. A canonical pointing at a dead URL is worse than the original problem, since crawlers follow it. Resolved by redeploying with **"Use existing Build Cache" disabled**.

Verified on the new alias after that rebuild: canonical correct on `/` and `/services`, `og:image` correct and serving a real `image/png`, `robots.txt` `Host`/`Sitemap` correct, `sitemap.xml` entries correct, the full security-header set present, `/admin` still gated (307), and the CV still downloading (200). The GitHub repository's homepage field was also updated, having still pointed at the deleted alias.

**Operational note for any future domain change:** updating `NEXT_PUBLIC_SITE_URL` is not sufficient on its own. `NEXT_PUBLIC_*` values are inlined during the build, so the change only takes effect on a rebuild that does **not** reuse the build cache.
